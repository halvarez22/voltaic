...
const ProjectCard: React.FC<{ project: Project }> = ({ project }) => (
  <div className="h-96 rounded-xl overflow-hidden shadow-lg bg-neutral-800 group transition-all duration-300 hover:shadow-2xl hover:shadow-brand-yellow/20">
    ...
    <div className="p-6">
      <div className="flex justify-between items-start mb-2">
        ...
        <span className="bg-brand-yellow/20 text-brand-yellow text-xs font-semibold px-3 py-1 rounded-full">
          {project.year}
        </span>
      </div>
      
      <p className="text-neutral-400 text-sm mb-4">{project.location}</p>
      ...
      <div className="flex justify-between items-center pt-4 border-t border-neutral-700">
        <div>
          ...
          <span className="text-brand-yellow font-bold">{project.capacity}</span>
        </div>
        <div className="bg-neutral-700/50 px-3 py-1 rounded-full">
...
const ProjectsSection: React.FC<{ id?: string }> = ({ id = 'proyectos' }) => {
  ...
  return (
    <section 
      id={id} 
      className="flex-shrink-0 w-screen h-full flex flex-col items-center p-6 relative overflow-hidden"
    >
      ...
      <div className="container mx-auto text-center relative z-10 flex flex-col h-full pt-20 md:pt-28">
        <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold mb-6 text-white">
          Nuestros <span className="text-brand-yellow">Proyectos</span>
        </h2>
        
        <p className="max-w-3xl mx-auto text-neutral-300 text-lg mb-8 px-4">
        ...
        <div className="flex-1 min-h-0 pb-12">
          {filteredProjects.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 h-full overflow-y-auto pr-4 vertical-scroll-section">
            ...
            <div className="h-full flex items-center justify-center">
              <p className="text-neutral-400">No hay proyectos en esta categoría.</p>
            </div>
          )}
        </div>
      </div>
